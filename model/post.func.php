...
function post_find_by_tid($tid, $page = 1, $pagesize = 50) {
	global $conf,$auth_config;
    $apiHTTP = new \SixparkSource\Oauth2\HTTPRequest($auth_config);
    $replyListJson = $apiHTTP->postWithAuth($auth_config['resource_host']."/index.php?app=index&act=replylist",['cid'=>$tid]);
    $replylist = json_decode($replyListJson,true);
