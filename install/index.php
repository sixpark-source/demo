...
if(empty($action)) {
	...
	if($method == 'GET') {
		$input = array();
		$input['lang'] = form_select('lang', array('zh-cn'=>'简体中文'), $conf['lang']);

		// 修改 conf.php
		include INSTALL_PATH."view/htm/index.htm";
	} else {
		$_lang = param('lang');
		!in_array($_lang, array('zh-cn')) AND $_lang = 'zh-cn';
	...
	if($method == 'GET') {
		...
		$token_route = param('token_route');
		$resource_host = param('resource_host');
		$authorize_route = param('authorize_route');

		empty($client_id) AND message('client_id', 'client_id 不能为空');
		empty($client_secret) AND message('client_secret', 'client_secret 不能为空');
		empty($token_route) AND message('token_route', 'Token获取服务器不能为空，一般默认即可');
		empty($resource_host) AND message('resource_host', '资源服务器地址不能为空，一般默认即可');
		empty($authorize_route) AND message('authorize_route', '用户认证服务器不能为空，一般默认即可');
		...
		file_replace_var(APP_PATH.'conf/conf.auth.php', $auth_config);
